						...
						<div class="flex items-start justify-between gap-3">
							<div class="text-sm leading-relaxed">
								{#if item.type === 'bet_created'}
									...
									<strong>{item.creator}</strong> started a bet
									<a href={`/app/bet/${item.betId}`} class="font-medium text-primary hover:underline">“{item.title}”</a>
									{#if item.participants.length > 1}
										<span class="text-muted-foreground"> with {nameList(item.participants.filter((p) => p !== item.creator))}</span>
									{/if}.
								...
								{:else if item.type === 'bet_cancelled'}
									...
									<strong>{item.cancelledBy}</strong> called off the bet
									<a href={`/app/bet/${item.betId}`} class="font-medium text-primary hover:underline">“{item.title}”</a>.
